...
contract DeployLocal is Script {
    ...
    function run() external returns (Deployment memory deployed) {
        ...
        _writeEnvironment(deployed, executorKey, localStartBlock);
    ...
    function _writeEnvironment(Deployment memory deployed, uint256 executorKey, uint256 localStartBlock) private {
        ...
        // The remaining contracts are intentionally absent from the minimal stack. Override their
        // zero-address template start blocks too, so Ponder never rescans old auto-mined blocks.
